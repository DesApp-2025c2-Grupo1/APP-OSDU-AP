...
function NotificationsBell() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/prestadores/notificaciones`, { credentials: 'include' })
      .then(res => res.json())
      .then(data => {
  ...
  async function openNotification(notification) {
    ...
    if (notification.unread) {
      await fetch(`${API_URL}/prestadores/notificaciones/${notification.id}/leida`, { method: 'PUT', credentials: 'include' }).catch(console.error)
    }
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:9002'
    fetch(`${API_URL}/prestadores/dashboard/stats`, { credentials: 'include' })
      .then(r => r.json())
      .then(d => setData(d))

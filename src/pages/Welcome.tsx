...
export function Welcome() {
  ...
  const heroTitle = esPrestadores ? (
    <>
      Tu gestión,<br />
      <span className="text-emerald-700">más</span> simple<br />
      y segura.
    </>
  ) : (
    <>
  ...
  return (
    <div className={`min-h-screen flex flex-col ${esPrestadores ? "bg-[#f7fbf8]" : "bg-white"}`}>
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 min-h-16 py-3 flex flex-wrap items-center justify-between gap-3 lg:gap-6">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-sm font-medium text-slate-600">
            ...
            <button className={`transition-colors ${esPrestadores ? "hover:text-emerald-700" : "hover:text-unahur"}`}>Beneficios</button>
          </nav>
          ...
          <div className="flex flex-1 items-center justify-end gap-2 sm:flex-none sm:gap-3">
              ...
              {sitioAbierto && (
                <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150 sm:left-auto sm:w-52">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-slate-50 transition-colors ${sitioActual === "prestadores" ? "text-emerald-700 font-semibold" : "text-slate-700"}`}
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>
            ...
            <button
              ...
              className={`hidden sm:block text-sm font-semibold px-5 py-2 rounded-xl transition-colors border ${esPrestadores ? "text-emerald-700 border-emerald-600/30 hover:bg-emerald-50" : "text-unahur border-unahur/30 hover:bg-green-50"}`}
            >
      ...
      {/* Hero Section */}
      <section className={`relative flex-1 overflow-hidden ${esPrestadores ? "bg-gradient-to-br from-slate-50 via-emerald-50/30 to-white" : "bg-gradient-to-br from-slate-50 via-green-50/30 to-white"}`}>
        {/* Decoración de fondo */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 md:py-16 lg:py-24 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-20">
            ...
            <div className="flex-1 text-center lg:text-left">
              <div className={`inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full mb-6 ${esPrestadores ? "bg-emerald-600/10 text-emerald-700" : "bg-unahur/10 text-unahur"}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                <button
                  onClick={() => navigate(loginPath)}
                  className={`flex items-center justify-center gap-2 px-8 py-4 text-white rounded-2xl font-bold text-base shadow-lg active:scale-[0.98] transition-all ${esPrestadores ? "bg-emerald-600 shadow-emerald-600/25 hover:bg-emerald-700" : "bg-unahur shadow-unahur/25 hover:bg-green-700"}`}
                >
                ...
                <button
                  onClick={() => esPrestadores ? setSitioActual("afiliados") : navigate("/register")}
                  className={`flex items-center justify-center gap-2 px-8 py-4 bg-white text-slate-800 border-2 border-slate-200 rounded-2xl font-bold text-base active:scale-[0.98] transition-all ${esPrestadores ? "hover:border-emerald-600/30 hover:bg-emerald-50" : "hover:border-unahur/30 hover:bg-green-50"}`}
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>

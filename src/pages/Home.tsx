...
export function Home() {
  ...
  const { activeProfile } = useOutletContext<{ activeProfile: Persona }>(); // 🔹 Recibimos el perfil desde el Layout
    ...
    <>
      <div className="mb-10 text-center">
        <div className="inline-block"> 
          ...
          <p className="text-gray-500 text-sm">
            Gestionando cuenta de: <span className="text-unahur font-semibold">{activeProfile.nombre} {activeProfile.apellido}</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {menuItems.map((item) => (
          <button 
            key={item.title}
            onClick={() => navigate(item.route)} // 🔹 Redirección al hacer click
